...
3. **총점은 모든 배점 항목의 합계에서 감점을 정확히 반영하여 계산하세요. 예: 20점 만점에서 5점 감점이면 15점이어야 합니다.**
...
7. **맞춤법 평가: OCR 오류로 인한 맞춤법 실수는 최대 1점까지만 감점하세요.**
8. **논리적 체계성 평가: 구조적 완벽성보다는 내용의 논리적 연결성을 중시하여 최대 1점까지만 감점하세요.**
...
- 논리적 체계성: 내용의 논리적 연결성과 일관성 (구조적 완벽성보다는 내용의 흐름 중시, 최대 1점 감점)
- 맞춤법: OCR의 한계로 인한 오류는 최대 1점까지만 감점 (예: '따리서' → '따라서' 같은 OCR 오류는 관대하게 평가)
...
JSON 형식(키와 타입을 반드시 지키세요):
{
  ...
  "categories": {
    "logicalStructure": number,    // 논리적 체계성 (최대 1점 감점, 내용의 논리적 연결성 중시) 0~10
    "spelling": number,           // 맞춤법 (OCR 오류 고려하여 최대 1점 감점) 0~10
    "vocabulary": number          // 어휘 및 문장의 적절성 (OCR로 인한 어색한 표현은 관대하게 평가) 0~10
  }
}

**중요: 점수 계산 시 반드시 문제의 배점 기준을 정확히 파악하고, 각 배점 항목별로 충족도를 평가하여 점수를 부여하세요. 감점을 정확히 반영하여 최종 점수를 계산하세요. 예: 20점 만점에서 5점 감점이면 15점이어야 합니다.**`
