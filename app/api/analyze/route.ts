...
export async function POST(request: Request) {
  try {
    ...
    const prompt = `당신은 교직논술 전문 평가자입니다. 아래 문제와 답안을 바탕으로 매우 상세하고 체계적으로 분석하여 점수와 피드백을 JSON으로만 반환하세요.
...
4. 강점과 보완점을 배점 기준과 논술 체계를 활용하여 구체적으로 제시하세요
5. 개선 방향은 배점 기준에 맞춰 실질적이고 구체적으로 제안하세요 (구체적인 예시나 문장 제안 포함)
6. 각 평가 항목에 대해 구체적인 근거와 예시를 포함하세요
...
JSON 형식(키와 타입을 반드시 지키세요):
{
  ...
  "strengths": string[],       // 배점 기준에 따른 강점 4~6개 (구체적 근거와 예시 포함)
  "weaknesses": string[],      // 배점 기준에 따른 보완점 4~6개 (구체적 문제점과 개선 필요성 포함)
  "improvements": string[],    // 배점 기준에 맞춘 구체적 개선방안 4~6개 (실행 가능한 방법론과 구체적 예시 포함)
  "detailedAnalysis": {
